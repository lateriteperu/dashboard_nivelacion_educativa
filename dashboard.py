...
def load_data():
    try:
        ...
        # Reemplazo nativo seguro de las etiquetas de sesión sobre las filas
...
if df_raw is not None:
    ...
    # =========================================================================
    # ✂️ SECCIÓN RECESO CONSOLIDADA: EXCLUSIÓN GLOBAL PARA GRÁFICOS Y MÉTRICAS
    # =========================================================================
    receso_inicio = pd.to_datetime('2026-05-17').date()
    receso_fin = pd.to_datetime('2026-05-31').date()
    
    # Aplicamos la exclusión estricta a ambas fuentes de datos
    ...
    with tab1:
        ...
        if not df_filtered.empty:
            ...
            if not df_filtered.empty:
                ...
                fig_asist.update_xaxes(type='date', tickformat='%d-%b', dtick="D1")
            ...
            if not df_filtered.empty:
                ...
                fig_total_asist.update_layout(
                    ...
                    hovermode="x unified",
                    yaxis_range=[0, df_sumas_diarias['Asistencia_Absoluta'].max() * 1.15]
                )
    ...
    with tab2:
        ...
        if not df_filtered.empty:
            ...
            m2.metric("Sesiones con Evaluación (%)", f"{pct_aplicacion:.1f}%", help="Porcentaje de sesiones realizadas que cuentan con un Exit Ticket registrado.")
            ...
            m4.metric("Estudiantes en Logro", f"{promedio_logro_real:.1f}%", help="Un estudiante alcanza el nivel de logro cuando responde correctamente el 80% o más del exit ticket.")
            ...
            df_melt = df_counts.melt(id_vars=['Date', 'Tema_Dictado'], value_vars=['Logro', 'Proceso', 'Inicio'], var_name='Nivel', value_name='Porcentaje')
    ...
    with tab3:
        ...
        if not df_final_talleres.empty:
            ...
            fig_taller = px.bar(
                df_asist_plot, x='Date', y='Asistencia_Absoluta', color='Institucion', barmode='group', text_auto=True,
                ...
                labels={'Tema_Taller': 'Tema del Taller', 'Asistencia_Absoluta': 'Estudiantes Asistentes', 'Institucion': 'Institución', 'Date': 'Fecha'}
            )
        ...
        if not df_final_talleres_cultural.empty:
            ...
            fig_cultural = px.bar(
                df_asist_plot_cultural, x='Date', y='Asistencia_Absoluta', color='Institucion', barmode='group', text_auto=True,
                title="Asistencia por institución, grado y fecha", color_discrete_map=colores_intensos,
                hover_data={'Tema_Taller': True, 'Asistencia_Absoluta': True, 'Institucion': True, 'Date': '|%d-%b'},
                labels={'Tema_Taller': 'Tema del Taller', 'Asistencia_Absoluta': 'Estudiantes Asistentes', 'Institucion': 'Institución', 'Date': 'Fecha'}
            )
